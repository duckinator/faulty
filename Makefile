...
linux: build_info
	mkdir -p build/linux
	${GODOT} src/project.godot ${EXPORT_FLAG} linux ../build/linux/faulty.x86_64

mac: build_info
	mkdir -p build/mac
	${GODOT} src/project.godot ${EXPORT_FLAG} macos ../build/mac/faulty_macos.zp
	mv build/mac/faulty_macos.zp build/mac/faulty_macos.zip
	cd build/mac && unzip faulty_macos.zip
	rm build/mac/faulty_macos.zip

windows: build_info
	mkdir -p build/windows
	${GODOT} src/project.godot ${EXPORT_FLAG} windows ../build/windows/faulty.exe
